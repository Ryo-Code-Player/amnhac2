...
@section('content')
<h2 class="intro-y text-lg font-medium mt-10">Danh sách bài hát của chủ đề</h2>
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <a href="{{ route('admin.topic_music.create') }}" class="btn btn-primary shadow-md mr-2">Thêm bài hát vào chủ đề</a>
    ...
    @else
        @foreach($topic_music_manage as $type)
        <tr class="intro-x">
            <td class="text-left">
                <a href="{{ route('admin.topic_music.show', $type->id) }}" class="font-medium whitespace-nowrap">{{ $type->id }}</a>
            </td>
            <td>
                <a href="{{ route('admin.topic_music.show', $type->id) }}" class="font-medium whitespace-nowrap">{{ $type->title }}</a>
            </td>
...
<div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
    <nav class="w-full sm:w-auto sm:mr-auto">
        {{ $topic_music_manage->links('vendor.pagination.tailwind') }}
    </nav>
</div>
<!-- END: Pagination -->
