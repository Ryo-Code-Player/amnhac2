...
class UserController extends Controller
{
    ...
    public function userSearch(Request $request)
    {
        ...
        {
            return redirect()->route('admin.user.index')->with('success','Không có thông tin tìm kiếm!');
        }
    ...
    public function moneyUserToStore($id)
    {
        ...
        if( $supplier)
        {
             ...
             $breadcrumb = '
             <li class="breadcrumb-item"><a href="#">/</a></li>
             <li class="breadcrumb-item  " aria-current="page"><a href="'.route('admin.user.index').'">Ds người dùng</a></li>
    ...
    public function moneySaveUserToStore(Request $request)
    {
        ...
        if( $supplier)
        {
            ...
            return redirect()->route('admin.user.showsup',$supplier->id)->with('success','Đã nạp tiền nhà cung cấp!');
        }
        else
        {
            return back()->with('error','Không tìm thấy dữ liệu');
    ...
    public function moneySaveStoreToUser(Request $request)
    {
        ...
        if( $supplier)
        {
            ...
            // \App\Models\Log::insertLog($content,$user->id);
            \App\Models\Log::insertLogNew($content,$subtrans->id,'supwi',$user->id);
            return redirect()->route('admin.user.showsup',$supplier->id)->with('success','Đã lưu!');
